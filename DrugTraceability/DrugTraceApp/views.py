...
from .models import Product, TraceRecord, UserProfile, Order
...
def chatbot_order(request):
    """
    Simple chatbot-like view to take orders from supply chain users (distributor, retailer, etc).
    The conversation is managed via session state.
    """
    if not request.user.is_authenticated:
        return redirect('login')

    step = request.session.get('chatbot_step', 1)
    context = {}
    ...
    if request.method == 'POST':
        if step == 1:
            ...
            try:
                ...
                request.session['chatbot_product_id'] = product.id
                request.session['chatbot_step'] = 2
                context['message'] = f"Selected product: {product.name}. How many units do you want to order?"
            except Product.DoesNotExist:
                context['error'] = "Invalid product selected."
        elif step == 2:
            ...
            try:
                ...
                if quantity <= 0:
                    raise ValueError
                product_id = request.session.get('chatbot_product_id')
                product = Product.objects.get(id=product_id)
                # Create the order
                ...
                request.session['chatbot_step'] = 1
                context['message'] = f"Order placed for {quantity} units of {product.name}. Thank you!"
            except (ValueError, Product.DoesNotExist):
                context['error'] = "Please enter a valid quantity."
    else:
        # Initial GET request
        request.session['chatbot_step'] = 1
        context['message'] = "Welcome to the Drug Order Chatbot! Please select a drug to order."

    step = request.session.get('chatbot_step', 1)
    if step == 1:
        context['products'] = Product.objects.all()
        return render(request, 'chatbot_order.html', context)
    elif step == 2:
        return render(request, 'chatbot_order_quantity.html', context)
